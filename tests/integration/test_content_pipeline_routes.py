...
def _override_focused_pipeline_services(
    ...
    *,
    ...
    smoke_proof_service: object | None = None,
) -> None:
    ...
    if smoke_proof_service is not None:
        app.dependency_overrides[get_pipeline_smoke_proof_service] = lambda: smoke_proof_service
...
# --- T04: search smoke-proof route tests -----------------------------------


@dataclass(slots=True)
class _RouteFakeQdrantSyncClient:
    """Qdrant sync double used by the smoke-proof route tests."""

    fetch_preview: object = None
    fetch_error: Exception | None = None
    fetch_calls: list[uuid.UUID] = field(default_factory=list)

    async def upsert_meme_point(self, payload: object, vector: tuple[float, ...]) -> None:
        _ = (payload, vector)

    async def fetch_meme_point(self, meme_file_id: uuid.UUID) -> object:
...
@dataclass(slots=True)
class _RouteFakeQdrantSimilarityClient:
    """Qdrant similarity double used by the smoke-proof route tests."""
    ...
    async def find_similar_memes(
        ...
        *,
        ...
        _ = (vector, current_meme_file_id, limit)
        if self.error is not None:
            raise self.error
        return self.matches
...
@dataclass(slots=True)
class _RouteFakeMeilisearchSyncClient:
    """Meilisearch sync double used by the smoke-proof route tests."""
    ...
    async def fetch_document(self, meme_file_id: uuid.UUID) -> object:
        _ = meme_file_id
        if self.fetch_error is not None:
            raise self.fetch_error
        return self.fetch_preview
...
def _build_qdrant_route_preview(meme_file_id: uuid.UUID) -> object:
    from memexpert.models.enums import SyncTargetKind as _SyncTargetKind
    from memexpert.schemas.content_pipeline import (
        ContentPipelineSyncTargetPreview as _ContentPipelineSyncTargetPreview,
    )

    return _ContentPipelineSyncTargetPreview(
        target=_SyncTargetKind.QDRANT,
        preview_fields={
...
def _build_meili_route_preview(meme_file_id: uuid.UUID) -> object:
    from memexpert.models.enums import SyncTargetKind as _SyncTargetKind
    from memexpert.schemas.content_pipeline import (
        ContentPipelineSyncTargetPreview as _ContentPipelineSyncTargetPreview,
    )
    ...
    return _ContentPipelineSyncTargetPreview(
        target=_SyncTargetKind.MEILISEARCH,
        preview_fields={"id": meme_file_id.hex},
        preview_fetched_at=utcnow(),
    )


async def _drive_to_dual_synced(
    session: AsyncSession,
    *,
    source_id: str,
    post_id: str,
...
async def test_pipeline_search_smoke_route_happy_path_returns_both_searchable(
    app: FastAPI,
    client: AsyncClient,
    migrated_db_session: AsyncSession,
) -> None:
    """Happy path: both targets searchable → 200 + both_targets_searchable=true."""
    ...
    meili = _RouteFakeMeilisearchSyncClient(
        fetch_preview=_build_meili_route_preview(meme_file_id),
        search_hits=[{"id": meme_file_id.hex}],
    )

    _override_focused_pipeline_services(app, smoke_proof_service=service)
    app.dependency_overrides[get_qdrant_sync_client] = lambda: qdrant_sync
    app.dependency_overrides[get_qdrant_similarity_client] = lambda: similarity
    app.dependency_overrides[get_meilisearch_sync_client] = lambda: meili
    ...
    assert response.status_code == 200
    body = response.json()
    assert body["both_targets_searchable"] is True
    targets = {entry["target"]: entry for entry in body["targets"]}
    assert targets["qdrant"]["searchable"] is True
    assert targets["qdrant"]["matched_by"] == "both"
    assert targets["meilisearch"]["searchable"] is True
    assert targets["meilisearch"]["matched_by"] == "both"


async def test_pipeline_search_smoke_route_returns_200_when_qdrant_point_missing(
    app: FastAPI,
    client: AsyncClient,
    migrated_db_session: AsyncSession,
) -> None:
    """Qdrant snapshot says synced but fetch_point returns None → 200, reason=point_not_found."""
    ...
    assert response.status_code == 200
    body = response.json()
    assert body["both_targets_searchable"] is False
    qdrant_entry = next(entry for entry in body["targets"] if entry["target"] == "qdrant")
    meili_entry = next(entry for entry in body["targets"] if entry["target"] == "meilisearch")
    assert qdrant_entry["searchable"] is False
    assert qdrant_entry["reason"] == "point_not_found"
    # Meilisearch must STILL be proved even when Qdrant failed.
    assert meili_entry["searchable"] is True


async def test_pipeline_search_smoke_route_returns_200_when_both_targets_missing(
    app: FastAPI,
    client: AsyncClient,
    migrated_db_session: AsyncSession,
) -> None:
    """Both targets return None → 200, both per-target searchable flags False."""
    ...
    qdrant_sync = _RouteFakeQdrantSyncClient(fetch_preview=None)
    similarity = _RouteFakeQdrantSimilarityClient()
    meili = _RouteFakeMeilisearchSyncClient(fetch_preview=None)
    ...
    assert body["both_targets_searchable"] is False
    targets = {entry["target"]: entry for entry in body["targets"]}
    assert targets["qdrant"]["searchable"] is False
    assert targets["qdrant"]["reason"] == "point_not_found"
    assert targets["meilisearch"]["searchable"] is False
    assert targets["meilisearch"]["reason"] == "document_not_found"


async def test_pipeline_search_smoke_route_rejects_blank_query(
    app: FastAPI,
    client: AsyncClient,
) -> None:
    """Blank query must fail request validation with HTTP 422."""
...
async def test_pipeline_search_smoke_route_rejects_missing_fields(
    app: FastAPI,
    client: AsyncClient,
) -> None:
    """Neither field set → 422 (exactly-one-of constraint)."""
...
async def test_pipeline_search_smoke_route_rejects_both_fields(
    app: FastAPI,
    client: AsyncClient,
) -> None:
    """Both fields set → 422 with clear exactly-one-of message."""
    ...
    detail_repr = json.dumps(payload)
    assert "not both" in detail_repr or "exactly one" in detail_repr


async def test_pipeline_search_smoke_route_returns_404_for_unknown_meme_file_id(
    app: FastAPI,
    client: AsyncClient,
    migrated_db_session: AsyncSession,
) -> None:
    """Unknown meme_file_id → 404 pipeline_item_not_found."""

    operator_token = get_settings().pipeline_operator_token.get_secret_value()
    service = PipelineSmokeProofService(migrated_db_session)
    qdrant_sync = _RouteFakeQdrantSyncClient()
    similarity = _RouteFakeQdrantSimilarityClient()
    meili = _RouteFakeMeilisearchSyncClient()
    unknown_id = uuid.uuid7()
    ...
    try:
        response = await client.post(
            "/api/v1/pipeline/search/smoke",
            headers={PIPELINE_OPERATOR_TOKEN_HEADER_NAME: operator_token},
            json={"meme_file_id": str(unknown_id)},
        )
    finally:
        app.dependency_overrides.clear()
...
async def test_pipeline_search_smoke_route_requires_operator_token(
    app: FastAPI,
    client: AsyncClient,
) -> None:
    """The search/smoke route must reject requests without the operator token header."""

    response = await client.post(
        "/api/v1/pipeline/search/smoke",
        json={"meme_file_id": str(uuid.uuid7())},
    )
    assert response.status_code == 401
    assert response.json()["code"] == "invalid_operator_token"


async def test_pipeline_search_smoke_route_registers_in_openapi(
    app: FastAPI,
    client: AsyncClient,
) -> None:
    """The search/smoke endpoint must appear in the generated OpenAPI schema."""

    response = await client.get("/openapi.json")
    paths = response.json()["paths"]
    assert "/api/v1/pipeline/search/smoke" in paths
    smoke_post = paths["/api/v1/pipeline/search/smoke"]["post"]
    assert any(
        parameter["name"] == PIPELINE_OPERATOR_TOKEN_HEADER_NAME
        and parameter["in"] == "header"
        for parameter in smoke_post["parameters"]
    )


async def test_pipeline_search_smoke_route_query_only_resolves_top_hit(
    app: FastAPI,
    client: AsyncClient,
    migrated_db_session: AsyncSession,
) -> None:
    """Query-only lookup must resolve the top Meilisearch hit and proof only that item."""
    ...
    # The query-resolve step hits the Meilisearch adapter first; its top hit
    # then becomes the id the per-item proof runs against. The search hits
    # list carries the meme_file_id hex as the top entry so the route resolves
    # the correct item.
    meili = _RouteFakeMeilisearchSyncClient(
        fetch_preview=_build_meili_route_preview(meme_file_id),
        search_hits=[{"id": meme_file_id.hex}],
    ...
    assert body["meme_file_id"] == str(meme_file_id)
    assert body["both_targets_searchable"] is True
    # The resolve step + the smoke proof should have each called search().
    assert len(meili.search_calls) >= 2
    # The resolve call is the first search; it uses the operator query.
    assert meili.search_calls[0]["query"] == "hello meme"


async def test_pipeline_search_smoke_route_query_only_returns_404_on_no_hits(
    app: FastAPI,
    client: AsyncClient,
    migrated_db_session: AsyncSession,
) -> None:
    """Query-only with no Meilisearch hits → 404 pipeline_item_not_found."""

    operator_token = get_settings().pipeline_operator_token.get_secret_value()
    service = PipelineSmokeProofService(migrated_db_session)
    qdrant_sync = _RouteFakeQdrantSyncClient()
    similarity = _RouteFakeQdrantSimilarityClient()
    meili = _RouteFakeMeilisearchSyncClient(search_hits=[])
