...
| `features/app-shell/GlobalSearch.svelte` | Global search entry | Provides a single query-only GET form on desktop and an explicit Search route entry on narrow screens. The desktop placeholder includes the public-safe catalog count when available; the root SSR load and sitemap share a short-lived server cache for that SEO summary. It does not own filter controls. |
