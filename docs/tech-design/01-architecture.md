...
| **Scheduler** | APScheduler process for periodic tasks: materialized-view refresh, popularity snapshots, like-count/index sync, trending, Meme of the Day, scheduled SEO batches. | Services → PG, Redis, RabbitMQ |
...
| **Scheduler** | APScheduler | Periodic tasks (MV refresh, trending, batch sync, popularity snapshots, MOTD, scheduled SEO) |
