...
- Full-account collection management: deterministic verified email/password accounts, private seeded collection page, invite accept, owner member-role update, editor invite controls without member management, collection-scoped search URL filters, and profile pin reorder confirmation.
...
- Connected-account settings, private uploads, and account-linking flows are still deferred; the browser suite now covers the NSFW opt-in boundary through guest session state and one seeded full-account collection collaboration path.
