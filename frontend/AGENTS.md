...
- Treat a committed browser mutation and successful route revalidation as separate outcomes. Loaders that expose `loadError` must not erase independently successful data with all-or-nothing empty fallbacks, and client code must not treat a resolved invalidation as proof that the mutated resource is present. Preserve last-known-good slices, apply authoritative mutation responses immediately when possible, prefer targeted `depends`/`invalidate`, and retry until the expected resource/version is observed.
