<script lang="ts">
  ...
  import { Badge, Button, FormRow, Input, Select } from '$lib/ui';
  ...
  let { source, telegramAccounts }: { source: AdminSourceChannelRead; telegramAccounts: AdminTelegramSessionRead[] } = $props();
...
<article class="grid gap-4 rounded-3xl border border-line bg-paper p-4">
  <div class="flex flex-wrap items-start justify-between gap-3">
    <div class="min-w-0">
      <h3 class="m-0 text-xl font-black tracking-[-0.03em]">{source.title}</h3>
      <p class="m-0 text-sm text-muted">{model.handleLabel}</p>
    </div>
    <div class="flex flex-wrap gap-2">
      <Badge>{model.platformLabel}</Badge>
      <Badge tone={model.status === 'Healthy' ? 'success' : 'neutral'}>{model.status}</Badge>
    </div>
  </div>

  <div class="grid gap-2 text-sm md:grid-cols-3">
    <p class="m-0"><strong>Last fetched:</strong> {model.lastFetchLabel.replace('Last fetched ', '')}</p>
    <p class="m-0"><strong>Account:</strong> {model.assignedAccountLabel}</p>
    <p class="m-0 text-muted">{model.statusDetail}</p>
  </div>
