...
{#if data.landing && page}
  <PageHeader title={data.landing.title} description={data.landing.description} badge="Tag page" />
  ...
  <Card class="mb-5 grid gap-3 shadow-none" aria-label="Tag trend summary">
    <h2 class="m-0 text-2xl font-black tracking-[-0.04em]">Tag trend</h2>
...
{:else}
  <EmptyState title="Tag unavailable" message={data.errorMessage}>
