...
import { ApiError, fetchAdminAnalyticsContent } from '$lib/api/client';
...
export const load: PageServerLoad = async ({ fetch, request, url }) => {
  ...
  try {
    const dashboard = await fetchAdminAnalyticsContent({
      fetch,
      baseUrl: apiBaseUrl(),
...
function analyticsLoadError(caught: unknown): string {
  return caught instanceof ApiError ? caught.message : 'Could not load content analytics.';
}
