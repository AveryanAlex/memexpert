# ruff: noqa: E501
"""use high-water source deltas in public trend read models
...
def upgrade() -> None:
    """Rebuild public trend views with high-water deltas and canonical sends."""

    _recreate_public_trend_materialized_views(legacy_semantics=False)


def downgrade() -> None:
    """Restore the adjacent-delta and inline-send-compatible view definitions."""

    _recreate_public_trend_materialized_views(legacy_semantics=True)
...
def _recreate_public_trend_materialized_views(*, legacy_semantics: bool) -> None:
    _drop_public_trend_materialized_views()
    definitions = (
        _PUBLIC_MEME_TRENDS_MV_SQL,
        _PUBLIC_TAG_TRENDS_MV_SQL,
        _PUBLIC_TEMPLATE_TRENDS_MV_SQL,
        _PUBLIC_TAG_TREND_POINTS_MV_SQL,
        _PUBLIC_TEMPLATE_TREND_POINTS_MV_SQL,
    )
    if legacy_semantics:
        definitions = tuple(_with_legacy_semantics(definition) for definition in definitions)

    meme_sql, tag_sql, template_sql, tag_points_sql, template_points_sql = definitions
    op.execute(meme_sql)
    ...
    recent_download_weight = " + recent_download_count * 2" if legacy_semantics else ""
    previous_download_weight = " + previous_download_count * 2" if legacy_semantics else ""
    op.execute(
        ...
        f"{recent_download_weight}) - (previous_view_count + previous_send_count * 3 "
        f"+ previous_like_count * 5 + previous_save_count * 4{previous_download_weight})) DESC, "
        "trending_score DESC, meme_id)"
    )
    op.execute(
    ...
    op.execute(tag_points_sql)
    op.execute(
        "CREATE UNIQUE INDEX uq_public_tag_trend_points_mv_tag_observed_at "
        "ON public_tag_trend_points_mv (tag, observed_at)"
    )
    op.execute(
        "CREATE INDEX ix_public_tag_trend_points_mv_observed_at "
        "ON public_tag_trend_points_mv (observed_at DESC, tag)"
    )
    op.execute(
        "CREATE INDEX ix_public_tag_trend_points_mv_value "
        "ON public_tag_trend_points_mv (aggregate_value DESC, observed_at DESC, tag)"
    )

    op.execute(template_points_sql)
    op.execute(
        "CREATE UNIQUE INDEX uq_public_template_trend_points_mv_template_observed_at "
        "ON public_template_trend_points_mv (template_id, observed_at)"
    )
    op.execute(
        "CREATE INDEX ix_public_template_trend_points_mv_slug_observed_at "
        "ON public_template_trend_points_mv (template_slug, observed_at)"
    )
    op.execute(
        "CREATE INDEX ix_public_template_trend_points_mv_observed_at "
        "ON public_template_trend_points_mv (observed_at DESC, template_slug)"
    )
    op.execute(
        "CREATE INDEX ix_public_template_trend_points_mv_value "
        "ON public_template_trend_points_mv (aggregate_value DESC, observed_at DESC, template_slug)"
    )


def _with_legacy_semantics(sql: str) -> str:
    sql = sql.replace(
        "'meme_send', 'share', 'meme_share'",
        "'meme_send', 'share', 'meme_share', 'inline_sent'",
    )
    for metric in ("view", "reaction", "forward"):
        sql = sql.replace(
            f"""max(ses.{metric}_count) OVER (
            PARTITION BY ses.meme_source_id
            ORDER BY ses.captured_at, ses.id
            ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING
        )""",
            f"""lag(ses.{metric}_count) OVER (
            PARTITION BY ses.meme_source_id
            ORDER BY ses.captured_at, ses.id
        )""",
        ).replace(f"previous_{metric}_high", f"previous_{metric}_count")
    for current, legacy in (
        ...
        sql = sql.replace(current, legacy)
    return sql
...
WITH safe_events AS (
    ...
    WHERE event_type::text IN (
        'meme_view', 'view', 'meme_send', 'share', 'meme_share',
...
event_counts AS (
    SELECT
        ...
        count(*) FILTER (WHERE event_type IN ('meme_send', 'share', 'meme_share') AND occurred_at >= now() - interval '7 days')::integer AS recent_platform_send_count,
        count(*) FILTER (WHERE event_type IN ('meme_send', 'share', 'meme_share') AND occurred_at >= now() - interval '14 days' AND occurred_at < now() - interval '7 days')::integer AS previous_platform_send_count,
        ...
        count(*) FILTER (WHERE event_type IN ('meme_send', 'share', 'meme_share') AND occurred_at >= now() - interval '24 hours')::integer AS platform_send_count_24h,
        ...
        count(*) FILTER (WHERE event_type IN ('meme_send', 'share', 'meme_share'))::integer AS latest_platform_sends,
...
source_deltas AS (
    SELECT
        meme_id,
        captured_at,
        CASE WHEN previous_view_high IS NULL OR view_count IS NULL THEN 0 ELSE GREATEST(view_count - previous_view_high, 0) END::integer AS view_delta,
        CASE WHEN previous_reaction_high IS NULL OR reaction_count IS NULL THEN 0 ELSE GREATEST(reaction_count - previous_reaction_high, 0) END::integer AS reaction_delta,
        CASE WHEN previous_forward_high IS NULL OR forward_count IS NULL THEN 0 ELSE GREATEST(forward_count - previous_forward_high, 0) END::integer AS forward_delta
...
WITH safe_events AS (
    SELECT
        ...
        occurred_at
    FROM analytics_events
    WHERE event_type::text IN ('meme_view', 'view', 'meme_send', 'share', 'meme_share', 'meme_like', 'favorite', 'meme_save', 'save')
),
event_daily AS (
    SELECT
        meme_id,
        date_trunc('day', occurred_at, 'UTC') AS observed_at,
        count(*) FILTER (WHERE event_type IN ('meme_view', 'view'))::integer AS platform_views,
        count(*) FILTER (WHERE event_type IN ('meme_send', 'share', 'meme_share'))::integer AS platform_sends,
    ...
    FROM safe_events
    WHERE meme_id IS NOT NULL
    GROUP BY meme_id, date_trunc('day', occurred_at, 'UTC')
),
successful_source_snapshots AS (
    SELECT
        mf.meme_id,
...
source_daily AS (
    SELECT
        meme_id,
        date_trunc('day', captured_at, 'UTC') AS observed_at,
        count(*)::integer AS snapshot_count,
        sum(CASE WHEN previous_view_high IS NULL OR view_count IS NULL THEN 0 ELSE GREATEST(view_count - previous_view_high, 0) END)::integer AS source_views,
        sum(CASE WHEN previous_reaction_high IS NULL OR reaction_count IS NULL THEN 0 ELSE GREATEST(reaction_count - previous_reaction_high, 0) END)::integer AS source_reactions,
        sum(CASE WHEN previous_forward_high IS NULL OR forward_count IS NULL THEN 0 ELSE GREATEST(forward_count - previous_forward_high, 0) END)::integer AS source_reposts
    FROM successful_source_snapshots
